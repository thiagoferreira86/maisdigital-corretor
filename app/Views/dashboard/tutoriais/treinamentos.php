...
$videos = Video::paginate($page,$quantity,$query,' id DESC');
$count = Video::count($query,"OTIMIZEvideos");
...
<html lang="pt-br">
	...
	<body id="kt_body" class="header-fixed header-mobile-fixed header-bottom-enabled page-loading">
						...
						<div class="d-flex flex-column-fluid">
							...
							<div class="container">
								...
								<div class="row">
								     <?php
                                        if(count($videos) ==0){
                                            echo '<div class="col-md-12"><p style="text-align:center;">Nenhum Vídeo encontrado.</p></div>';
                                        } else {
                                            foreach($videos as $v){ 
                                    ?>
                                	<div class="col-xl-4">
                                        ...
                                        <!--begin::Body-->
                                            <div class="card-body treinamento">
                                                <?=$v->iframe?>
                                                <h2><?=$v->titulo?></h2>
                                                <p><?=$v->subtitulo?></p>
                                            </div>
                                                ...
                                                <div class="d-flex justify-content-between align-items-center flex-wrap">
                                                    <div class="d-flex flex-wrap py-2 mr-3">
                                                        <?php
                                                            ...
                                                            if($pg > 0){ ?>
                                                                <a href="<?=DASHBOARD?>capacitacao/treinamentos/<?=$pg?>" class="btn btn-icon btn-sm btn-light-danger mr-2 my-1"><i class="ki ki-bold-arrow-back icon-xs"></i></a>
                                                            <? } else { ?>
                                                            ...
                                                            <? }
                                                                ...
                                                                if($pg == ($i_pg-1)){ ?>
                                                                    <? if($i_pg != 1){ ?>
                                                                         <a href="<?=DASHBOARD?>capacitacao/treinamentos/<?=$i_pg-1?>" class="btn btn-icon btn-sm border-0 btn-hover-danger mr-2 my-1"><?=$i_pg-1?></a>
                                                                    <? } ?>
                                                                    <a href="javascript:void(0)" class="btn btn-icon btn-sm border-0 btn-hover-danger active mr-2 my-1"><?=$i_pg?></a>
                                                            <?  } else if($i_pg-1 < $pages){
                                                                    $i_pg2 = $i_pg-1; ?>
                                                                   
                                                                     <a href="<?=DASHBOARD?>capacitacao/treinamentos/<?=$i_pg2+1?>" class="btn btn-icon btn-sm border-0 btn-hover-danger mr-2 my-1"><?=$i_pg?></a>
                                                            <?  }
                                                            }
                                                            if(($pg+2) < $quant_pg){ ?>
                                                                <a href="<?=DASHBOARD?>capacitacao/treinamentos/<?=$pg+2?>" class="btn btn-icon btn-sm btn-light-danger mr-2 my-1"><i class="ki ki-bold-arrow-next icon-xs"></i></a>
                                                        <? } else { ?>
		...
		<!--end::Page Scripts-->
        <!--begin::Page Scripts(used by this page)-->
        <script src="assets/js/pages/features/calendar/basic.js?v=7.2.9"></script>
        <script>
            ...
            function ativarLanding(id){
                ...
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "dashboard/tutoriais/statusLanding",
                ...
                Swal.fire({
                    ...
                    cancelButtonText: "Não, cancelar!",
                            ...
                            $.ajax({
                                type: "POST",
                                dataType: "json",
                                url: "dashboard/tutoriais/excluiPagina",
                                data: { id:id },
                                processData: true,
                                success: function(data){
                                    console.log(data)
