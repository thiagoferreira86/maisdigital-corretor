...
class CartaoDigitalController{
    public function seusCartoes(): void
    {
        $ativa = 'Cartão Digital';
        $subativo = 'Seus Cartões';
        $corretoraNome = explode(' ', $_SESSION['corretora_nome']);

        $this->view('cartao-digital/seus-cartoes', compact(
            'ativa', 'subativo', 'corretoraNome'            
        ));
    }
    public function cartao(): void
    {
        $ativa = 'Cartão Digital';
        $subativo = 'Cartão';
        $corretoraNome = explode(' ', $_SESSION['corretora_nome']);

        $this->view('cartao-digital/cartao', compact(
            'ativa', 'subativo', 'corretoraNome'            
        ));
    }
    public function modelosProntos(): void
    {
        $ativa = 'Cartão Digital';
        $subativo = 'Modelos Prontos';
        $corretoraNome = explode(' ', $_SESSION['corretora_nome']);

        $this->view('cartao-digital/modelos-prontos', compact(
            'ativa', 'subativo', 'corretoraNome'            
        ));
    }
    public function cartaoPreview(): void
    {
        $ativa = 'Cartão Digital';
        $subativo = 'Cartão Preview';
        $corretoraNome = explode(' ', $_SESSION['corretora_nome']);

        $this->view('cartao-digital/cartao-preview', compact(
            'ativa', 'subativo', 'corretoraNome'            
        ));
    }
